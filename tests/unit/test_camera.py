...
class test_Camera:
    ...
    def test_get_heartbeat(self):
        ...
        assert self.cam.get_heartbeat(worker) == make_aware(datetime.fromtimestamp(t3))
    ...
    def test_handle_task_timezone(self):
        ...
        with override_settings(USE_TZ=True, TIME_ZONE="Europe/Helsinki"):
            ...
            mt = self.cam.handle_task((task.uuid, task))
            assert mt.tstamp == datetime(2016, 6, 1, 15, 0, 0, tzinfo=dt_timezone.utc)
            assert mt.eta == datetime(
                2016, 6, 1, 15, 16, 17, 654321, tzinfo=dt_timezone.utc
            )
            assert mt.expires == datetime(
                2016, 7, 1, 12, 16, 17, 765432, tzinfo=dt_timezone.utc
            )
            ...
            mt = self.cam.handle_task((task.uuid, task))
            assert mt.eta == datetime(
                2016, 6, 4, 15, 16, 17, 654321, tzinfo=dt_timezone.utc
            )
    ...
    def test_on_shutter(self):
        ...
        events = [
            ...
            Event(
                "task-failed", uuid=uus[1], exception="KeyError('foo')", hostname=ws[1]
            ),
